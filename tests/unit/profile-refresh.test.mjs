...
const repo=(name,stars,extra={})=>({name,owner:{login:'wz20'},stargazers_count:stars,updated_at:'2026-01-01T00:00:00Z',...extra});
...
test('ties use update date, then deterministic repository name',()=>{
  assert.deepEqual(rank([repo('b',1),repo('a',1),repo('recent',1,{updated_at:'2026-09-07T00:00:00Z'})]).map(r=>r.name),['recent','a','b']);
});
