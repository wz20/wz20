...
- **PENDING — updated PR-head render:** after the local ordering commit is pushed by an authorized release step, GitHub's sanitized dark/light desktop/mobile rendering, exact project order, link counts, image loading, and overflow must be re-verified on that new head. No result is estimated or inferred from the superseded render.
